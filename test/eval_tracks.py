...
def main(directory):
    ...
    for fn in sorted(os.listdir(absdir)):
        ...
        if os.path.isfile(filename) and re_rpps_fn.match(fn):
            ...
            df = pandas.read_csv(filename, delimiter=';', names=practitioner.RPPS.KEYS_CAT18, header=0, index_col=False)
            ...
            for domain in MSSANTE:
                for dom in domain['domains']['value']:
                    ...
                    data[domain['name']]['mssante'][dom].append(dict(date=_date.strftime("%Y-%m-%d"), 
                        value=int(_count)))
